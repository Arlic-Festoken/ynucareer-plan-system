...
- 低年级首次建档后完成一次“方向校准”，保存兴趣、动机、问题场景与宽方向，随后退出长期主导航。
- DeepSeek 把方向画像作为当前账号内、可修改的规划上下文，而不是训练或微调数据；AI 规划是唯一的新计划生成入口。
- AI 规划页用“方向画像 → AI 细分 → 行动计划 → 行动中心”解释完整链路，并在个人资料中提供重新校准入口。
- 重新校准会标记旧 AI 计划需要更新，但不会删除已有行动、完成状态、反思、证据或教师反馈。
...
- 低年级 AI 规划与探索计划自动融合为一份计划：AI 作为主线，不重复的探索行动作为补充，已完成、已反思或有成果的记录进入历史成果。
...
- 画像、方向选择、AI 规划上下文与结果、培养方案、学习路径、研究生双线计划和本地行动草稿会在每次修改时立即写入当前账号专属的浏览器快照，并在短暂防抖后同步到 SQLite `career_states`。
- 重新打开、刷新或换设备登录时优先恢复该账号的完整规划。未同步的本机快照不会被较旧云端数据覆盖，不同账号也不会复用同一份本地规划。
- 云端暂时不可用时，顶部状态会显示“已保存在本机”并允许重试；退出登录不会删除这份待同步快照。浏览器禁止本地存储时会明确告警，并继续使用账号云端同步。
- `GET /api/me/career-state` 同时返回 `state` 与服务端 `updatedAt`，不需要修改现有 schema 8。

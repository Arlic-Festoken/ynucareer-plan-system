...
export default function MatchingPage() {
  ...
  return <PageShell eyebrow="高年级决策" title="先选一个参照目标，再把差距变成行动。" description="这里的准备度只用于自我诊断。它不是录取、求职或任何未来结果的概率。">
    ...
    {profile.targetPath !== "employment" ? <section className="alternative-path"><div><span className="section-kicker">当前路径 / {pathway.label}</span><h2>{pathway.description}</h2><ol>{pathway.tasks.map((task) => <li key={task}>{task}</li>)}</ol></div><aside><span className="signal-label">下一步</span><strong>生成本学期行动计划</strong><p>计划可以继续修改，完成后请把成果和复盘留下来。</p><button className="button button-primary" onClick={generate} type="button">生成{pathway.label}行动计划 <ArrowRight size={16} /></button>{roadmapTasks.length > 0 && <Link className="button button-quiet" to="/student/roadmap">查看已生成计划</Link>}</aside></section> : <>
      <section className="diagnosis-layout"><div className="target-picker"><span className="section-kicker">选择一个参照岗位</span><h2>选择一个想靠近的岗位</h2><p>这不是承诺。它只是帮助你识别应该优先积累哪类证据。</p><div className="job-discovery-controls"><label className="job-search"><span><Search size={16} />岗位搜索</span><div><input onChange={(event) => setQuery(event.target.value)} placeholder="搜索岗位、行业或工作内容" type="search" value={query} />{query && <button aria-label="清空岗位搜索" onClick={() => setQuery("")} type="button"><X size={15} /></button>}</div></label><label className="job-industry-filter"><span>行业场景</span><select aria-label="行业场景" onChange={(event) => setSelectedIndustry(event.target.value)} value={selectedIndustry}>{industryFilters.map((industry) => <option key={industry}>{industry}</option>)}</select></label><small aria-live="polite">{visibleJobs.length} 个结果</small></div><div className="job-options">{visibleJobs.map((item) => <button className={item.id === job.id ? "is-selected" : ""} key={item.id} onClick={() => setSelectedJobId(item.id)} type="button"><span>{item.industry}</span><strong>{item.title}</strong><p>{item.description}</p></button>)}{visibleJobs.length === 0 && <div className="job-empty"><Search size={20} /><strong>没有找到匹配岗位</strong><p>换一个关键词或行业场景，查看更宽的结果。</p><button className="button button-quiet" onClick={clearJobFilters} type="button">清除筛选</button></div>}</div></div>

...
export default function OnboardingPage() {
  ...
  const majorOptions = stored.major && !majors.includes(stored.major) ? [stored.major, ...majors] : majors;
  ...
  const [major, setMajor] = useState(stored.major || majors[0]);
  ...
  async function submit(event: React.FormEvent) {
    ...
    try {
      ...
      await saveProfile({ university: accountProfile?.university || "云南大学", college: accountProfile?.college || "", major, grade, bio: accountProfile?.bio || "" });
      await syncCareerNow();
      navigate(resolveHome(profile.role, profile.grade));
    } catch {
      setError("资料同步失败，请稍后重试。");
    } finally {
      setSubmitting(false);
    }
  }

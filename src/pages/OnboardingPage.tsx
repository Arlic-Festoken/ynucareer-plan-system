...
export default function OnboardingPage() {
  ...
  function submit(event: React.FormEvent) {
    ...
    if (!interests.length || !values.length) { setError("请至少选一个感兴趣的方向和一个你在意的价值。它们只用于生成本地建议。"); return; }
  ...
  return <PageShell>
    <section className="setup-layout"><aside className="setup-aside"><Link className="site-brand compact" to="/"><span className="brand-square">→</span>向前</Link><div><span className="section-kicker">开始之前</span><h1>用两分钟，<br />让建议从你出发。</h1><p>这里不会问姓名、成绩或联系方式。所有内容仅保存在当前浏览器。</p></div><ol><li className={step === 1 ? "is-active" : ""}><span>01</span>确认阶段</li><li className={step === 2 ? "is-active" : ""}><span>02</span>给出起点</li></ol><Link className="aside-link" to="/teacher/dashboard">我是教师，查看模拟洞察 <ArrowRight size={15} /></Link></aside>
      <section className="setup-main"><div className="setup-progress"><span>第 {step} 步 / 2</span><i><b style={{ width: `${step * 50}%` }} /></i></div>
        {step === 1 ? <div className="setup-step"><span className="section-kicker">选择你的当下</span><h2>你现在最需要解决什么？</h2><p>系统会据此安排接下来的主任务，不会把所有内容同时塞给你。</p><div className="stage-choice-grid">{roles.map((item) => <label className={role === item.role ? "stage-choice is-selected" : "stage-choice"} key={item.role}><input checked={role === item.role} name="role" onChange={() => selectRole(item.role)} type="radio" /><span className="stage-choice-icon">{item.role === "freshman" ? <Compass size={22} /> : item.role === "junior" ? <GraduationCap size={22} /> : <Sparkles size={22} />}</span><span><strong>{item.title}</strong><small>{item.caption}</small><em>{item.description}</em></span><Check size={18} /></label>)}</div><div className="setup-actions"><Link className="button button-quiet" to="/">返回首页</Link><button className="button button-primary" onClick={() => setStep(2)} type="button">继续 <ArrowRight size={17} /></button></div></div> : <form className="setup-step" onSubmit={submit}><span className="section-kicker">给建议一点上下文</span><h2>你想从哪里开始？</h2><p>这些不是档案，只用于把第一件行动安排得更贴近你。</p><div className="form-grid"><label>当前年级<select onChange={(event) => setGrade(Number(event.target.value))} value={grade}>{grades.map((item) => <option key={item} value={item}>{role === "graduate" ? `研 ${item - 4}` : `大 ${item}`}</option>)}</select></label><label>专业或学习领域<select onChange={(event) => setMajor(event.target.value)} value={major}>{majors.map((item) => <option key={item}>{item}</option>)}</select></label>{role !== "freshman" && <label>本次优先路径<select onChange={(event) => setTargetPath(event.target.value as Pathway)} value={targetPath}>{pathways.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}</select></label>}</div><fieldset><legend>我愿意先接近的方向</legend><div className="option-chips">{interestOptions.map((item) => <label key={item}><input checked={interests.includes(item)} onChange={() => toggle(setInterests, interests, item)} type="checkbox" /><span>{item}</span></label>)}</div></fieldset><fieldset><legend>我更在意什么</legend><div className="option-chips">{valueOptions.map((item) => <label key={item}><input checked={values.includes(item)} onChange={() => toggle(setValues, values, item)} type="checkbox" /><span>{item}</span></label>)}</div></fieldset>{error && <p className="form-error" role="alert">{error}</p>}<div className="setup-actions"><button className="button button-quiet" onClick={() => setStep(1)} type="button"><ArrowLeft size={17} />上一步</button><button className="button button-primary" type="submit">生成我的行动计划 <ArrowRight size={17} /></button></div></form>}
      </section></section>
  </PageShell>;
}

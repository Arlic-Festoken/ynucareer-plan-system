...
export default function GraduatePage() {
  ...
  function saveResearchStart(event: React.FormEvent) {
    event.preventDefault();
    if (!research.focus.trim() || !research.industry.trim()) {
      setPlanError("请先填写研究方向和希望靠近的产业或场景。");
      setStartSaved(false);
      return;
    }
    setPlanError("");
  ...
  return <PageShell eyebrow="研究生导航" title="让研究成果既能留下来，也能走出去。" description="把科研与职业准备放进同一份双线计划。">
    ...
    <section className="graduate-input-grid"><article><Beaker size={22} /><span className="section-kicker">研究起点</span><h2>我正在研究什么</h2><p className="input-helper">这两项输入会决定双线计划里的研究问题、产业场景和下一步任务。</p><form className="research-start-form" onSubmit={saveResearchStart}><label>研究方向<input aria-invalid={Boolean(planError && !research.focus.trim())} onChange={(event) => { setResearch({ focus: event.target.value }); setPlanError(""); setStartSaved(false); }} placeholder="例如：面向学习分析的生成式 AI" value={research.focus} /></label><label>希望靠近的产业或场景<input aria-invalid={Boolean(planError && !research.industry.trim())} onChange={(event) => { setResearch({ industry: event.target.value }); setPlanError(""); setStartSaved(false); }} placeholder="例如：教育科技、智慧医疗或智能制造" value={research.industry} /></label><div className="research-context-list">{researchIndustries.map((item) => <div key={item.research}><strong>{item.research}</strong><span>{item.industries.join(" · ")}</span></div>)}</div><div className="research-start-actions"><button className="button button-secondary" type="submit">保存研究起点</button>{startSaved && <span role="status">已保存，可生成双线计划。</span>}</div></form></article><article><Link2 size={22} /><span className="section-kicker">成果证据</span><h2>我已经做成了什么</h2><form className="outcome-capture" onSubmit={addOutcome}><select aria-label="成果类型" onChange={(event) => setOutcomeType(event.target.value as typeof outcomeType)} value={outcomeType}>{Object.entries(researchOutcomeLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select><input aria-label="成果名称" onChange={(event) => setOutcomeTitle(event.target.value)} placeholder="例如：完成学习行为预测项目" required value={outcomeTitle} /><button className="button button-secondary" type="submit"><Plus size={16} />添加</button></form><div className="outcome-cards">{research.outcomes.length ? research.outcomes.map((outcome) => <div key={outcome.id}><span>{researchOutcomeLabels[outcome.type]}</span><strong>{outcome.title}</strong><button aria-label={`删除 ${outcome.title}`} onClick={() => removeResearchOutcome(outcome.id)} type="button"><Trash2 size={15} /></button></div>) : <p>先记录正在推进的项目、论文或比赛。</p>}</div></article></section>

...
export default function StudentHomePage() {
  ...
  return <PageShell>
    ...
    <section className="signal-metric-grid"><SignalMetric detail={explorer ? "用行动验证方向。" : "可随时重新计算。"} label={explorer ? "探索状态" : "计划状态"} value={explorer ? `${awakening.activeStep}/6` : `${completed}/${tasks.length || 0}`} /><SignalMetric detail={explorer ? "来自兴趣与真实体验。" : "来自课程、项目与实践。"} label="你正在积累" tone="neutral" value={explorer ? "方向证据" : "能力证据"} /><SignalMetric detail="仅存当前浏览器。" label="数据边界" tone="warm" value="本地保存" /></section>

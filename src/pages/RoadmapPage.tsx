...
export default function RoadmapPage() {
  ...
  return <PageShell eyebrow={explorer ? "探索行动" : "行动计划"} title="让计划进入真实的时间里。" description="完成一件，记录发现，再安排下一步。">
    <section className="roadmap-overview"><div><Route size={27} /><div><span className="section-kicker">本阶段</span><h2>{tasks.length ? "你的行动正在积累证据" : "先生成一份有起点的计划"}</h2><p>{tasks.length ? "按时间推进，完成后可复盘。" : explorer ? "先完成六步方向探索。" : "先完成一次目标诊断。"}</p></div></div>{tasks.length > 0 && <ProgressRail current={completed} label="已完成" total={tasks.length} />}</section>

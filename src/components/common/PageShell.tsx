import { BarChart3, Bot, ClipboardCheck, Compass, FlaskConical, Home, RotateCcw, Sparkles, UserRound } from "lucide-react";
...
export default function PageShell({ children, eyebrow, title, description, mode }: PageShellProps) {
  ...
  const privateNav = isTeacher
    ...
    ? [{ to: "/graduate/navigation", label: "双线计划", icon: FlaskConical }, { to: "/onboarding", label: "调整画像", icon: Sparkles }, { to: "/account/profile", label: "个人资料", icon: UserRound }]
    : profile.role === "teacher"
      ? [{ to: "/teacher/dashboard", label: "模拟洞察", icon: BarChart3 }]
      : [{ to: studentHome, label: "工作台", icon: Compass }, { to: profile.grade <= 2 ? "/student/awakening" : "/student/matching", label: profile.grade <= 2 ? "探索方向" : "目标诊断", icon: Sparkles }, { to: "/student/ai-planning", label: "AI 规划", icon: Bot }, { to: "/student/roadmap", label: "行动计划", icon: ClipboardCheck }, { to: "/account/profile", label: "个人资料", icon: UserRound }];

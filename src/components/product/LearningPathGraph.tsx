    ...
    <div className="learning-path-graph" role="region" aria-label="算法工程师学习路径拓扑图" tabIndex={0}>
      ...
      <g className="graph-nodes">{nodes.map((item) => {
        ...
        >
          ...
          <text className="graph-node-kind" x="16" y="24">{item.kind.toUpperCase()}</text>
          {lines.map((line, index) => <text className="graph-node-title" key={line} x="16" y={52 + index * 20}>{line}</text>)}
          <text className="graph-node-semester" x="16" y="88">{item.semester}</text>
          <text className="graph-node-duration" x="16" y="104">预计 {item.durationWeeks} 周</text>
